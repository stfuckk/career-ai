...
- career_path — индивидуальный путь карьерного роста: текущая позиция пользователя и 3 последовательных ступени роста. Каждая ступень содержит название позиции, 3–4 конкретных навыка для перехода, требуемый опыт и поисковый запрос для hh.ru. Шаги должны быть реалистичными и логично выстроенными от стартовой позиции к более старшим ролям.
...
class BothubAIClient:
    ...
    def _build_user_message(
        ...
        *,
        ...
        schema = {
            ...
            'career_path': {
                'current_position': 'string (текущая позиция пользователя, 2–5 слов)',
                'steps': [
                    {
                        'title': 'string (название следующей позиции)',
                        'skills_to_learn': ['string (3–4 конкретных навыка или технологии)'],
                        'experience_required': 'string (например: без опыта / 1–3 года / от 5 лет)',
                        'hh_search_query': 'string (поисковый запрос для hh.ru для этой позиции)',
                    }
                ],
            },
        }
    ...
    def _normalize_response(
        ...
        career_path = self._normalize_career_path(response.get('career_path'))
    ...
    def _normalize_career_path(self, value: Any) -> dict[str, Any]:
        if not isinstance(value, dict):
            raise ValueError('career_path must be an object')
        current_position = self._require_non_empty_string(value.get('current_position'), 'career_path.current_position')
        raw_steps = value.get('steps')
        if not isinstance(raw_steps, list) or len(raw_steps) == 0:
            raise ValueError('career_path.steps must be a non-empty list')
        steps = []
        for i, step in enumerate(raw_steps[:4]):
            if not isinstance(step, dict):
                raise ValueError(f'career_path.steps[{i}] must be an object')
            title = self._require_non_empty_string(step.get('title'), f'career_path.steps[{i}].title')
            skills = self._ensure_string_list(step.get('skills_to_learn'), field_name=f'career_path.steps[{i}].skills_to_learn', min_items=1)
            experience_required = self._require_non_empty_string(step.get('experience_required'), f'career_path.steps[{i}].experience_required')
            hh_query = self._require_non_empty_string(step.get('hh_search_query'), f'career_path.steps[{i}].hh_search_query')
            steps.append({
                'title': title,
                'skills_to_learn': skills[:4],
                'experience_required': experience_required,
                'hh_search_query': hh_query,
            })
        return {'current_position': current_position, 'steps': steps}
